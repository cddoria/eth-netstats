...
Node.prototype.startWeb3Connection = function()
{
	...
	web3.setProvider( new web3.providers.HttpProvider('http://' + (process.env.RPC_HOST || '9.37.228.37') + ':' + (process.env.RPC_PORT || '8110')) );
...
client.on('connection', function(clientSpark)
{
	...
	{
		clientSpark.emit('init', { nodes: statz /*Nodes.all()*/ });
		/*
		clientSpark.write({
			action: 'charts',
...
/////////////////////////////////////////////////////////////
			client.emit('add', {nodes: self});

			if( !_.isUndefined(self.id) && !_.isUndefined(self.info) )
			{
				/*
				data.ip = self.address.ip;
				data.id = self.id;
				data.latency = N.latency;
				*/
				var info = Nodes.add(self);
				//console.log(info);
				client.emit('ready');
			...
			if( !_.isUndefined(self.id) && !_.isUndefined(self.stats) )
			{
			  //self.stats.latency = this.latency;
				client.emit('update', {nodes: self});
				var stat = {};
				stat = Nodes.update(info.id, info.stats);
				client.emit('ready');
				if(stat !== false)
				{
					client.write({
